...
std::string parse_type(int type)
{
    std::string str;
...
void Image_File_Write(sensor_msgs::Image::ConstPtr image_ptr, std::string str_name)
{
	...
	{		
		...
		if( !image.empty() )
		{			
			std::string str = parse_type(image.type());
			
			printf("image : %d, %s\n", image.channels(), parse_type(image.type()).c_str()) ;
...
int main(int argc, char **argv) 
{
	...
	int size_bags = vec_str_path_src.size() ;
	
	printf("\n\nBag file list -----\n") ;
	printf("count = %d\n", size_bags ) ;
	for( int i=0 ; i<size_bags ; i++ )
	{
		printf(" - [%d] %s\n", i, vec_str_path_src[i].c_str() ) ;
	}
	printf("-----\n") ;

	for( int i=0 ; i<size_bags ; i++ )
	{
		std::string str_bag_file = vec_str_path_src[i] ;
		...
		try
		{
			bag.open(str_bag_file, rosbag::bagmode::Read);  // BagMode is Read by default
			printf("read : %s\n", str_bag_file.c_str()) ;
			...
			printf("\n\nChecked %d topics from %s file ----- \n", size_topic_list, str_bag_file.c_str()) ;
			...
			//기본 파일 정보 
			int ifind = str_bag_file.rfind('/') ;
			std::string str_base_info ;
			if( ifind >= 0 )
			{
				int len = str_bag_file.size() - ifind ;
				str_base_info = str_bag_file.substr(ifind+1, len) ;
			}	
			...
			if( !str_base_info.empty() )
			{
				...
				//폴더 생성
				std::string str_write_base_path = str_path_dst + "/" + str_only_file_name ;
				if(!boost::filesystem::exists(str_write_base_path))
				{
					//Create Directory
					boost::filesystem::path directory(str_write_base_path);
				...
				for(rosbag::MessageInstance const m: rosbag::View(bag))
				{
					...
					{				
						...
						if( dbl_save_time_duration >= duration ||  duration < 0 )
						{
							//시간 간격을 체크하기 위한 저장 시간 저장
							it->second = dbl_mytime ;
			
							std::string str_write_path = str_write_base_path + "/" + str_only_file_name + "_" + str_sensor_name + "_" + std::to_string(dbl_mytime) + ".png" ;
				...
				std::string str_csv_write_path = str_write_base_path + "/" + str_only_file_name + ".csv" ;
